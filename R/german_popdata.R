...
german_popdata <- function(raw, clean) {
    ...
    dt[, c(cols) := lapply(.SD, function(x) nafill(x, "nocb")),
       keyby = .(region, agegroup), .SDcols = cols]
...
clean_pop <- function(path, file = c("12411-03-02-4.csv", "12411-03-03-4.csv")) {
    region_type <- region <- . <- agegroup <- NULL
    all_b <- ger_b <- for_b <- AGS <- NULL
    
    shp <- data.table(sf::read_sf(file.path(path, "shapes/districts_ext.shp")))
    
    file <- match.arg(file)
    dt <- read_pop(path, file)
    new_cols <- c("year", "region", "region_type", "agegroup",
                  "all_b", "all_m", "all_f",
                  "ger_b", "ger_m", "ger_f",
                  "for_b", "for_m", "for_f")
    ...
    dt <- dt[, .(year, region, agegroup, all_b, ger_b, for_b)]
    dt <- dt[region %in% shp[, AGS]] # filter all non districts

    dt <- dt[, c("all_b", "ger_b", "for_b") := lapply(.SD, sum),
             keyby = .(year, region, agegroup), .SDcols = c("all_b", "ger_b", "for_b")]

    dt <- dt[, .SD[1], keyby = .(year, region, agegroup)]
    dt <- dt[, .(region, year, agegroup,
                 all = all_b, german = ger_b, foreign = for_b)]

    return(dt)
}

rec_ages_ <- function(dt, file) {
    . <- agegroup <- new_age <- i.new_age <- old_age <- NULL
    last <- ifelse(file == "12411-03-02-4.csv", 2, 6)
    rec <- dt[, .(old_age = unique(agegroup))]
    ## a <- c("unter18", "18-25", "25-30", "30-50", "50-65", "\u00fcber65" )
    a <- c("below18", "18-25", "25-30", "30-50", "50-65", "above65" )
    new_ages <- c(NA, rep(a[1], 5), rep(a[2], 2), a[3], rep(a[4], 4), rep(a[5], 3), rep(a[6], last), "all")
    rec[, new_age := new_ages]
    dt[rec, agegroup := i.new_age, on = .(agegroup = old_age)]
    setnames(dt, "agegroup", "agegroup")
    return(dt)
}
